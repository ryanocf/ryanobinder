...
global SAMP_PPOOL_TEXTLABEL_OFFSET := 0xC
...
setWeatherID(id) {
    ...
    writeMemory(hGTA, 0xC81320, id, 2, "byte")
...
getTime() {
    ...
    current_time := readMem(hGTA, 0xB70153, 1, "UChar")
...
setTime(hour) {
    ...
    patchTime()
    writeMemory(hGTA, 0xB70153, hour, 1, "UChar")
...
NOP(hProcess, dwAddress, dwLen) {
    ...
    writeRaw(hProcess, dwAddress, &byteCode, dwLen)
...
getWeatherName() {
    ...
    return oweatherNames[id]
}

toggleFPSlock(status) {
    global instruction
    if (status)
    {
        instruction := readDWORD(hGTA, dwSAMP + 0x9D9D0)
        writeMemory(hGTA, dwSAMP + 0x9D9D0, 0x5051FF15, 4, "UChar")
    } else {
        if (instruction)
        {
            writeMemory(hGTA, dwSAMP + 0x9D9D0, instruction, 4, "UInt")
        }
    }
    return
}
...
getPlayerHealth() {
    ...
    ErrorLevel := ERROR_OK
    health := Round(fHealth)
    if (health >= 8000000)
        return health - 8000000
    else
        return health
}
...
getVehicleLightState() {
    if (!checkHandles())
        return 0
    Light := readMem(hGTA, 0xC3F0D0, 4, "UInt")
    if (ErrorLevel) {
        ErrorLevel := ERROR_READ_MEMORY
        return 0
    }
...
closeDialog() {
    if (!checkHandles())
        return false
    callWithParams2(hGTA, dwSAMP + 0x6B210, [["i", readDWORD(hGTA, dwSAMP + SAMP_DIALOG_STRUCT_PTR)]], false, true)
}
...
readDwordMulti(hProcess, dwAddress, offsets)
{
...
writeStringMulti(hProcess, dwAddress, offsets, wString) {
    if (!hProcess) {
        ErrorLevel := ERROR_INVALID_HANDLE
        return false
    }
    ...
    for key, value in offsets
    {
        if (A_Index == offsets.MaxIndex()) {
            StrPut(sString, &buffer, StrLen(sString) + 1)
            dwRet := DllCall( "WriteProcessMemory"
            , "UInt", hProcess
            , "UInt", dwAddress + value
            , "Str", buffer
...
; internal stuff
Memory_ReadByte(hProcess, address) {
    VarSetCapacity(value, 1, 0)
    DllCall("ReadProcessMemory", "UInt", hProcess, "UInt", address, "Str", value, "UInt", 1, "UInt *", 0)
...
Organizer(message) {
    AddChatMessage("{5C646C}[Hitman-Binder] {FFFFFF}" message "")
}

Exception(message) {
    AddChatMessage("{5C646C}[Hitman-Binder] FEHLER!{FFFFFF} " message "{5C646C}!")
}
...
URLDownloadToVar(url, showerror := false) {
    ...
    try {
        ...
        hObject.SetRequestHeader("User-Agent", "AutoHotkey")
...
convertvar(var) {
    ...
    StringReplace, var, var, [name], %name%, All
    StringReplace, var, var, [id], %id%, All
    StringReplace, var, var, [zone], %zone%, All
    StringReplace, var, var, [city], %city%, All
    StringReplace, var, var, [hp], %hp%, All
    StringReplace, var, var, [armor], %armor%, All
    StringReplace, var, var, [fps], %fps%, All
    StringReplace, var, var, [dl], %dl%, All
    StringReplace, var, var, [car], %car%, All
    StringReplace, var, var, [wanteds], %wanteds%, All
    StringReplace, var, var, [kills], %Kills%, ALL
    StringReplace, var, var, [dkills], %dKills%, ALL
    StringReplace, var, var, [tode], %Tode%, ALL
    StringReplace, var, var, [dtode], %dTode%, ALL
    return var
}
...
wb_BeforeNavigate2(wb, url, flags, frame, postdata, headers, cancel){
    global OnlineGui
    global TextbindGui
    global AdminpanelGui
    global UserpanelGui
    global LogpanelGui
    global WaffenteileGui
    ...
    if (m1 = "go" OR m1 = "g"){ 
        loop, Parse, m2, `;
        {
            if (IsLabel(A_LoopField) AND (wb != OnlineGui OR wb != TextbindGui OR AdminpanelGui OR wb != UserpanelGui OR wb != LogpanelGui OR wb != WaffenteileGui))
                gosub %A_LoopField% 
        }
    }
    return
...
getChatLine(line) {
    ...
    chat := readString(hGTA, dwAddress + SAMP_CHAT_OFF + SIZE_SAMP_CHATMSG * (99 - line), SAMP_CHAT_SIZE)
...
read_stats(array)
{
    ...
    for key, value in output
    {
        ...
        {
            if (RegExMatch(value, _value "\:\s\[(.*?)\]", rs_)) {
                output_object[_value] := rs_1
            }
        }
    }
...
deldollar(stringdollar) {
    IfInString,stringdollar,$
    {
        StringReplace, stringdollar, stringdollar,$,, All
    }
    return stringdollar
}
...
getStreamedVehicles() {
    ...
    dwAddress := readDWORD(hGTA, dwSAMP + SAMP_INFO_OFFSET)
    dwAddress := readDWORD(hGTA, dwAddress + SAMP_PPOOLS_OFFSET)
    stVehiclePool := readDWORD(hGTA, dwAddress + ADDR_SAMP_VEHPOOL)
    vehicleoffset := 0
...
getVehiclePointer(wID) {
    ...
    stVehiclePool := readDWORD(hGTA, var2 + ADDR_SAMP_VEHPOOL)
    if (stVehiclePool) {
        var3 := readDWORD(hGTA, stVehiclePool + VEHPOOL_pGTA_Vehicle + wID * 0x4)
        return var3
    } else {
        return false
    }
...
getVehicleNumberPlateById(wID) {
    ...
    vehpool := readDWORD(hGTA, dwAddress + ADDR_SAMP_VEHPOOL)
    if (ErrorLevel || vehpool==0) {
        ErrorLevel := ERROR_READ_MEMORY
        return ""
    }
    listed := readDWORD(hGTA, vehpool + VEHPOOL_iIsListed + wID*4)
    if (ErrorLevel || listed==0) {
        ErrorLevel := ERROR_READ_MEMORY
        return ""
    }
    svehptr := readDWORD(hGTA, vehpool + VEHPOOL_pGTA_Vehicle + wID*4)
    if (ErrorLevel) {
        ErrorLevel := ERROR_READ_MEMORY
        return ""
    }
...
printTextDraws() {
    if (!updateTextDrawData())
        return
    oReplace := ["~s~", "~r~", "~w~", "~h~", "~g~", "~y~", "~n~", "~b~", " ", " "]
    For i, o in oTextDrawData
    {
...
getNearestLabel(text := "") {
    ...
    for i, o in oTextLabelData {
        if (text != "" && o.TEXT != text)
            continue
        newDist := getDist(pos1, [o.XPOS, o.YPOS, o.ZPOS])
...
isVehicleAPlane() {
    ...
    Loop % oAirplaneModels.MaxIndex()
    {
        if (oAirplaneModels[A_Index] == getVehicleModelID())
        {
            return true
        }
    }
...
isVehicleABike() {
    ...
    {
        if (oMotorBikes[A_Index] == getVehicleModelID())
        {
            return true
        }
    }
...
maxranges() {
    ...
    NumPut(3, num, 0, "UInt")
    writeRaw(hGTA, 0xB6F0DC, &num, 4)
    writeRaw(hGTA, 0xB6F0F0, &num, 4)
    return true
}

getWeapon(weapon_arr, dialog_arr)
{
    for key, value in dialog_arr
    {
        weapon := ""
        if (RegExMatch(value, "^(.*)\s(\d+)\s(\d+)$", weapon_)) {
            for _key, weapon in weapon_arr
            {
                if (weapon = weapon_1)
                {
                    step := key - 2
...
readTextDraw(text)
{
    if (!checkHandles() || !updateTextDrawData())
        return false
    
    dwTextDraws := readDwordMulti(hGTA, dwSAMP, [SAMP_INFO_OFFSET, SAMP_PPOOLS_OFFSET, SAMP_PPOOL_TEXTDRAW_OFFSET])
    if (!dwTextDraws)
        return false
    ...
    {
        dwAddress := readDword(hGTA, dwTextDraws + ((A_Index - 1) * 4 + (4 * (SAMP_MAX_PLAYERTEXTDRAWS + SAMP_MAX_TEXTDRAWS * 2))))
        if (!dwAddress)
            continue
        
        string := readString(hGTA, dwAddress, 800)
        if (text != "") {
...
toggle_cursor(bToggle) {
    if (!checkHandles())
        return false
    
    dwAddress := readDWORD(hGTA, dwSAMP + 0x21A10C)
    callWithParams2(hGTA, dwSAMP + 0x9BD30, [["i", dwAddress], ["i", bToggle ? 3 : 0], ["i", !bToggle]], false, true)
    if (!bToggle)
        callWithParams2(hGTA, dwSAMP + 0x9BC10, [["i", dwAddress]], false, true)
